...
**Compile harness:** write generated code to a temp dir and typecheck with the peer SDK on disk. Browser-ts installs `@ht-sdks/events-sdk-js-browser` as a **CLI devDependency** (see `package.json`) and runs `ts.createProgram`. For other languages:

- Add the peer SDK as a CLI devDependency **or** check out the `ht-sdks` repo listed above, whichever actually typechecks.
...
Use `cliPackage()` from `src/lib/package-info.ts`. Do not hardcode the CLI version.
...
- Do not `npm install` / pin the peer SDK from generated output.
