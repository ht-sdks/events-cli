...
describe('renderGo', () => {
  ...
  it('keeps acronyms in payload type names as pascal case instead of rewriting to all caps', async () => {
    ...
    const src = await renderGo([event]);
    expect(src).toMatch(/type TrackJsonKeyProbeDefaultPayload struct/);
    expect(src).toContain(
      'func TrackJsonKeyProbeDefault(client htevents.Client, userID string, props TrackJsonKeyProbeDefaultPayload',
    );
