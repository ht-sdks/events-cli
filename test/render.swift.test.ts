...
describe('renderSwift', () => {
  it.each(['simple-track.json', 'multi-version.json', 'with-refs.json'])(
    ...
    async (file) => {
      const src = await renderSwift(eventsFromFixture(file));
  ...
  it('sorts events by wrapperName', async () => {
    ...
    const src = await renderSwift(events);
    const v1 = src.indexOf('func trackOrderCompletedV1(');
    const v2 = src.indexOf('func trackOrderCompletedV2(');
